...
public final class FindMeetingQueryTest {
  ...
  @Test
  public void everyAttendeeOptionalAllDay() {
    // Same as everyAttendeeIsConsidered but an optional has all day event
    // Should return as if the optional was not invited

    Collection<Event> events = Arrays.asList(
        new Event("Event 1", TimeRange.fromStartDuration(TIME_0800AM, DURATION_30_MINUTES), Arrays.asList(PERSON_A)),
        new Event("Event 2", TimeRange.fromStartDuration(TIME_0900AM, DURATION_30_MINUTES), Arrays.asList(PERSON_B)),
        new Event("Event 3", TimeRange.fromStartEnd(TimeRange.START_OF_DAY, TimeRange.END_OF_DAY, false), Arrays.asList(PERSON_C)));
  ...
  @Test
  public void everyAttendeeOptionalEarlyLate() {
    // Same as everyAttendeeIsConsidered but an optional an early event
    // Should return early and late parts of day
  ...
  public void justEnoughRoomOptional() {
    // Based on justEnoughRoom, optional should be ignored
    ...
    MeetingRequest request = new MeetingRequest(Arrays.asList(PERSON_A), DURATION_30_MINUTES, Arrays.asList(PERSON_B));
  ...
  public void onlyOptional() {
    ...
    Collection<Event> events = Arrays.asList(
        ...
        new Event("Event 2", TimeRange.fromStartEnd(TimeRange.START_OF_DAY, TIME_0800AM, false), Arrays.asList(PERSON_B)),
        new Event("Event 5", TimeRange.fromStartEnd(TIME_0900AM, TimeRange.END_OF_DAY + 1, false), Arrays.asList(PERSON_C)));
    
    MeetingRequest request = new MeetingRequest(DURATION_30_MINUTES, Arrays.asList(PERSON_A, PERSON_B, PERSON_C));
  ...
  public void optionalNoGaps() {
    ...
    Collection<Event> events = Arrays.asList(
        new Event("Event 1", TimeRange.fromStartEnd(TimeRange.START_OF_DAY, TIME_0800AM, false), Arrays.asList(PERSON_A)),
        new Event("Event 2", TimeRange.fromStartEnd(TIME_0800AM, TimeRange.END_OF_DAY + 1, false), Arrays.asList(PERSON_B)));
    
    MeetingRequest request = new MeetingRequest(DURATION_30_MINUTES, Arrays.asList(PERSON_A, PERSON_B));
